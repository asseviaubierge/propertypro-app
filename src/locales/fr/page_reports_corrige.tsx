...
export default function ReportsPage() {
  ...
  const fetchReport = useCallback(async (tab: string, range: string) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error("Échec du chargement du rapport");
  ...
  const handleExport = () => {
    ...
    switch (activeTab) {
      case "income-expense":
        ...
        filename = "income-expense-rapport";
        break;
      case "rent-roll":
        data = rentRollData;
        filename = "rent-roll-rapport";
        break;
      case "collections":
        data = collectionsData;
        filename = "collections-rapport";
        break;
    }
    if (!data) return;
            ...
            <SelectContent>
              {DATE_RANGES.map((r) => (
                <SelectItem key={r.value} value={r.value}>
                  {t(r.label,{defaultValue:r.value})}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
      ...
      <Tabs value={activeTab} onValueChange={handleTabChange}>
        <TabsList className="grid w-full grid-cols-3">
          <TabsTrigger value="income-expense" className="gap-2">
            ...
            <span className="hidden sm:inline">{t("reports.tabs.incomeExpense",{defaultValue:"Revenus & dépenses"})}</span>
          </TabsTrigger>
          <TabsTrigger value="rent-roll" className="gap-2">
            <Building2 className="h-4 w-4" />
            <span className="hidden sm:inline">{t("reports.tabs.rentRoll",{defaultValue:"État locatif"})}</span>
          </TabsTrigger>
          <TabsTrigger value="collections" className="gap-2">
            <DollarSign className="h-4 w-4" />
            <span className="hidden sm:inline">{t("reports.tabs.collections",{defaultValue:"Recouvrements"})}</span>
          </TabsTrigger>
        </TabsList>
                ...
                <tbody>
                  {incomeByProperty.map((item) => (
                    <tr key={item?._id || item?.propertyName || "property"} className="border-b last:border-0">
                      <td className="py-2">{item?.propertyName || "Bien inconnu"}</td>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {collectionsByProperty.map((item) => (
                    <tr key={item?._id || item?.propertyName || "property"} className="border-b last:border-0">
                      <td className="py-2 font-medium">{item?.propertyName || "Bien inconnu"}</td>
