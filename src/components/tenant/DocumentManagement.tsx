...
const getTypeBadge = (
  ...
  > = {
    lease: { label: typeLabelMap.lease, variant: "default" },
    notice: { label: typeLabelMap.notice, variant: "destructive" },
    maintenance: { label: typeLabelMap.maintenance, variant: "secondary" },
    insurance: { label: typeLabelMap.insurance, variant: "secondary" },
    receipt: { label: typeLabelMap.receipt, variant: "outline" },
    identification: { label: "ID", variant: "outline" },
    income: { label: typeLabelMap.income, variant: "outline" },
    inspection: { label: typeLabelMap.inspection, variant: "outline" },
    other: { label: typeLabelMap.other, variant: "secondary" },
  };

  const config = badgeConfig[normalized] ?? badgeConfig.other;
  const translatedLabel =
    normalized === "identification"
      ? t("leases.documents.types.identificationShort", {
          defaultValue: config.label,
        })
      : t(`leases.documents.types.${normalized}`, {
          defaultValue: config.label,
        });

  return <Badge variant={config.variant}>{translatedLabel}</Badge>;
};
...
export default function DocumentManagement({
  ...
  const formatLocalizedDate = useCallback(
    (value?: string) => {
      if (!value) {
        return t("leases.documents.labels.notAvailable", { defaultValue: "—" });
      }
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              {isLoading ? (
                ...
                <>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {t("leases.documents.summary.totalSize", {
                      defaultValue: "{size} total size",
                      values: { size: formatFileSize(totalFileSize) },
                    })}
                  </p>
                </>
              )}
          ...
          {summaryCategoryCards.map(([category, count]) => (
            <Card key={category}>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">
                  {t(`leases.documents.categories.${category}`, {
                    defaultValue:
                ...
                {categoryOptions.map((option) => (
                  <SelectItem key={option} value={option}>
                    {t(`leases.documents.categories.${option}`, {
                      defaultValue: categoryLabelMap[option] ?? toTitleCase(option),
                    })}
                  </SelectItem>
                ))}
              </SelectContent>
                ...
                {typeOptions.map((option) => (
                  <SelectItem key={option} value={option}>
                    {t(`leases.documents.types.${option}`, {
                      defaultValue: typeLabelMap[option] ?? toTitleCase(option),
                    })}
                  </SelectItem>
                ))}
              </SelectContent>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            {t("leases.documents.table.title", {
              defaultValue: "Documents ({count})",
              values: { count: filteredDocuments.length },
            })}
          </CardTitle>
          <CardDescription>
              ...
              <div className="rounded-md border">
                <Table>
                  <TableHeader>
                    <TableRow>
                      <TableHead>
                        {t("leases.documents.table.columns.document", {
                          defaultValue: "Document",
                        })}
                      </TableHead>
                      <TableHead>
                        {t("leases.documents.table.columns.type", {
                          defaultValue: "Type",
                        })}
                      </TableHead>
                      <TableHead>
                      ...
                      <TableHead className="text-right">
                        {t("leases.documents.table.columns.actions", {
                  ...
                  <TableBody>
                    {visibleDocuments.map((document) => (
                      <TableRow key={document._id}>
                        ...
                        <TableCell>{getTypeBadge(document.typeValue, t)}</TableCell>
                        <TableCell>
                          <div className="flex items-center gap-1">
                            <Building2 className="h-3 w-3 text-muted-foreground" />
                            {document.propertyLabel ||
                              t("leases.documents.labels.notAvailable", {
                                defaultValue: "—",
                              })}
                          </div>
                        </TableCell>
                        <TableCell>
                          <div className="flex items-center gap-1">
                            ...
                            {formatLocalizedDate(document.uploadedAt)}
                          </div>
                        </TableCell>
              ...
              <div className="rounded-lg border bg-muted/40 p-4">
                <div className="flex flex-col gap-2">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {getTypeBadge(selectedDocument.typeValue, t)}
                  </div>
