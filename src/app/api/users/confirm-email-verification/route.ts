...
export async function POST(request: NextRequest) {
  try {
    ...
    // Prépare la seconde étape : la vérification WhatsApp par E-IMMO.
    // Le statut devient "pending" uniquement lorsque l'utilisateur clique
    // réellement sur « Vérifier mon WhatsApp ».
    const verificationCode = `WA-${crypto
      .randomBytes(3)
      .toString("hex")
      .toUpperCase()}`;
    ...
    if (user.whatsappVerificationStatus !== "verified") {
      user.whatsappVerificationStatus = "not_requested";
      user.whatsappVerificationRequestedAt = null;
