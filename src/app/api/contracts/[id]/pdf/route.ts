...
export const GET = withAccessAndDB({})(
  async (user: any, _req: NextRequest, context: any) => {
    ...
    if (user.systemRole !== UserRole.ADMIN && String((contract.accountId as any)?._id || contract.accountId) !== String(user.id)) {
      return Response.json({ success: false, error: "Vous n'êtes pas autorisé à consulter ce document." }, { status: 403 });
    }
    ...
    if (contract.signatureStatus === "signed") {
      body += `\n\nSIGNATURE ÉLECTRONIQUE DU CONTRACTANT\nStatut : Signé\nSignataire : ${contract.signatoryName || accountName}\nDate : ${contract.signedAt ? new Date(contract.signedAt).toLocaleString("fr-FR") : "—"}\nLe contractant a confirmé avoir lu le document avant signature.`;
    } else if (contract.signatureStatus === "pending_signature") {
