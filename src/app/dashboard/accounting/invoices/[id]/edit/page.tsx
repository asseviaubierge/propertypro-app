...
function normalizeStatus(value?: string | null): InvoiceEditFormData["status"] {
  if (value && VALID_INVOICE_STATUSES.has(value)) {
    return value as InvoiceEditFormData["status"];
  }
  return "issued";
}

function normalizeLineItemType(
  value?: string | null,
): InvoiceEditFormData["lineItems"][number]["type"] {
  if (value && VALID_LINE_ITEM_TYPES.has(value)) {
    return value as InvoiceEditFormData["lineItems"][number]["type"];
  }
  return "other";
}
  ...
  const fetchInvoice = async () => {
    try {
      ...
      if (data?.success && data?.data) {
        const invoiceData = data.data as Invoice;
        ...
        form.reset({
          ...
          dueDate: toInputDate(invoiceData?.dueDate),
          issueDate: toInputDate(invoiceData?.issueDate),
                    ...
                    <>
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <span className="font-medium text-red-600">
                          ${(totalAmount - (invoice.amountPaid || 0)).toFixed(2)}
                        </span>
                      </div>
                    </>
                  )}
